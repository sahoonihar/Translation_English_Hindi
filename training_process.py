...
for epoch in range(num_epochs):
    ...
    if save_model and val_mean_loss == np.min(loss_tracker):
        ...
        save_checkpoint(MODEL_PATH, checkpoint, filename=f'model_{num_layers}_{vocab_size}_{embedding_size}.pth.tar')
    else:
        checkpoint = {
            "state_dict": model2.state_dict(),
            "optimizer": optimizer.state_dict(),
  }
        save_checkpoint(MODEL_PATH, checkpoint, filename=f'overfit_model_{num_layers}_{vocab_size}_{embedding_size}.pth.tar')
    
    if (epoch - early_stop_chk) >= 5:
...
pd.DataFrame(loss_dict).to_csv(f"./Loss/loss_{num_layers}_{vocab_size}_{embedding_size}.csv")
